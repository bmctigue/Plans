...
class PlannableProtocolTests: XCTestCase, Plannable {
    ...
    func testSetAmountForPlanInRegion() {
        let plan = PlanFactory.sharedInstance.create(plan1Hash)
    ...
    func testUpdatePlansWithAmount() {
        let plan1 = PlanFactory.sharedInstance.create(plan1Hash)
        let plan2 = PlanFactory.sharedInstance.create(plan2Hash)
